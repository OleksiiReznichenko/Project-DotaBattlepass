...
const { isAwardsWindowOpened, isRouletteWindowOpened, eventAction, awardsClosedIndicator } = storeToRefs(baseStore);
...
const idleFrames = ref(importAll(import.meta.glob("/src/assets/sprites/Outworld_Devourer/Idle/*")));
...
const playNext = (name, infinite) => {
  ...
  switch (name) {
    ...
    case "death":
      ...
      _deathAnimation.value?.play(6000, infinite);
...
onMounted(() => {
  _idleAnimation.value = new TileAnimation(canvasDestroyer.value, 900, 900, idleFrames.value);
